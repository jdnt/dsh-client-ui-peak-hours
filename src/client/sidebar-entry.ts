...
import { classifyPeak, formatCountdown, resolveTimeZone, type PeakState } from './time.ts'
...
function createEntry(): { entry: HTMLDivElement; label: HTMLSpanElement; countdown: HTMLSpanElement } {
  ...
  if (entry.parentElement !== root) {
    ...
    const family = Array.from(root.children).filter(
      (el): el is HTMLElement => el instanceof HTMLElement && el.matches(
        '[data-dsh-peakhours-entry], [data-dsh-taskboard-entry], [data-dsh-ssh-entry]',
      ),
    )
 ...
 */
export function mountSidebarEntry(source: PeakHoursEntrySource): () => void {
  ...
  const render = (state: PeakState): void => {
  ...
  const tick = (): void => { render(classifyPeak(new Date(), resolveTimeZone(source.getTimeZone()))) }
